<?php
	...
	if ( !empty($_POST)) {
		...
		$sql = "update `impuestos` set `nombre`=?, `es_fijo`=?, `valor`=? where id = ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($_POST['nombre'],$_POST['es_fijo'],$_POST['valor'],$id));
		...
		$pdo = Database::connect();
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "SELECT `id`, `nombre`, `es_fijo`, valor FROM `impuestos` WHERE  `id` = ? ";
		$q = $pdo->prepare($sql);
		$q->execute(array($id));
...
<body>
	...
	<div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Modificar Impuesto</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-body">
                            <div class="row">
                                <div class="col-lg-6">
                                    <form class="form-horizontal" action="modificarImpuesto.php?id=<?php echo $id?>" method="post">
									...
									<br><br>
									<button onclick="document.location.href='impuestos.php'" class="btn btn-default">Volver</button>
                                </div>
                                <!-- /.col-lg-6 (nested) -->
                            </div>
                            <!-- /.row (nested) -->
