<?php
	...
	if ( !empty($_POST)) {
		...
		$sql = "update `reglas` set `nombre`=?, `condicion`=?, `accion`=?, `recursiva`=?, `activa`=? where id = ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($_POST['nombre'],$_POST['condicion'],$_POST['accion'],$_POST['recursiva'],$_POST['activa'],$id));
		...
		$pdo = Database::connect();
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "SELECT `id`, `nombre`, `condicion`, `accion`, `recursiva`, activa FROM `reglas` WHERE  `id` = ? ";
		$q = $pdo->prepare($sql);
		$q->execute(array($id));
...
<body>
	...
	<div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Modificar Regla</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-body">
                            <div class="row">
                                <div class="col-lg-6">
                                    <form class="form-horizontal" action="modificarRegla.php?id=<?php echo $id?>" method="post">
										...
										<div class="form-group">
											...
											<textarea class="form-control" required="required" name="condicion" id="condicion"><?php echo $data['condicion']; ?></textarea>
											<br>
											Agregar Impuesto: 
										...
										<div class="form-group">
                                            <label>Acción</label>
											<textarea class="form-control" required="required" name="accion" id="accion"><?php echo $data['accion']; ?></textarea>
											<br>
											Agregar Impuesto: 
										...
										<div class="form-group">
											...
											<option value="1" <?php if ($data['recursiva']==1) echo " selected ";?>>Si</option>
											<option value="0" <?php if ($data['recursiva']==0) echo " selected ";?>>No</option>
											</select>
										</div>
										<div class="form-group">
                                            <label>Activa</label>
