<?php
	...
	if ( !empty($_POST)) {
		...
		$sql = "delete from `imagenes_publicaciones` where id_publicacion = ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($id));
		...
		for($i=0;$i<$countfiles;$i++){
			...
			move_uploaded_file($_FILES['imagenes']['tmp_name'][$i],'publicaciones/'.$id.'_'.$filename);
			
			$sql = "insert into `imagenes_publicaciones` (id_publicacion,imagen) values (?,?)";
			$q = $pdo->prepare($sql);
			$q->execute(array($id,$id.'_'.$filename));
		}
...
<body>
	...
	<div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Imágenes Publicación</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-body">
                            <div class="row">
                                <div class="col-lg-12">
                                    <form class="form-horizontal" action="imagenesPublicacion.php?id=<?php echo $id?>" method="post" enctype="multipart/form-data">
										<div class="form-group">
											<input type="hidden" name="algo" value="1">
											<input name="imagenes[]" type="file" multiple="multiple" class="form-control">
										</div>
